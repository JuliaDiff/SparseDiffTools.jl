    ...
    if dx isa Nothing
        ...
        tup =
            ArrayInterfaceCore.allowed_getindex(
                ArrayInterfaceCore.allowed_getindex(p, 1),
                1,
            ) .* false
        _pi = adapt(parameterless_type(dx), [tup for i = 1:length(dx)])
        fx = reshape(
...
generate_chunked_partials(x, colorvec, N::Integer) =
...
function generate_chunked_partials(x, colorvec, cs::Val{chunksize}) where {chunksize}
    ...
    for i = 1:maxcolor, j = 1:length(colorvec)
...
function forwarddiff_color_jacobian(
    f::F,
    ...
    chunksize = nothing,
    dx = sparsity === nothing && jac_prototype === nothing ? nothing : copy(x),
) where {F} #if dx is nothing, we will estimate dx at the cost of a function call
    ...
    return forwarddiff_color_jacobian(
        f,
        x,
...
) where {F} #dx kwarg can be used to avoid re-allocating dx every time
    if sparsity === nothing && jac_prototype === nothing
        cfg =
            chunksize === nothing ? ForwardDiff.JacobianConfig(f, x) :
            ForwardDiff.JacobianConfig(f, x, ForwardDiff.Chunk(getsize(chunksize)))
        return ForwardDiff.jacobian(f, x, cfg)
    end
    return forwarddiff_color_jacobian(
...
function forwarddiff_color_jacobian(
    ...
    if jac_prototype isa Nothing ? ArrayInterfaceCore.ismutable(x) :
       ArrayInterfaceCore.ismutable(jac_prototype)
        ...
        J =
            jac_prototype isa Nothing ?
            (
                sparsity isa Nothing ? false .* vec(dx) .* vecx' :
                zeros(eltype(x), size(sparsity))
            ) : zero(jac_prototype)
...
function forwarddiff_color_jacobian(
    ...
    if !(sparsity isa Nothing)
        ...
        rows_index = [rows_index[i] for i = 1:length(rows_index)]
        cols_index = [cols_index[i] for i = 1:length(cols_index)]
    end
    ...
    for i in eachindex(p)
        ...
        if !(sparsity isa Nothing)
            for j = 1:chunksize
                ...
                pick_inds =
                    [i for i in 1:length(rows_index) if colorvec[cols_index[i]] == color_i]
                ...
                if J isa SparseMatrixCSC || j > 1
                    ...
                    cols_index_c =
                        vcat(cols_index_c, zeros(Int, nrows - len_rows))[perm_rows]
                    Ji = [
                        j == cols_index_c[i] ? dx[i] : false for i = 1:nrows, j = 1:ncols
                    ]
                end
                if j == 1 && i == 1
...
function forwarddiff_color_jacobian_immutable(
    f,
    x::AbstractArray{<:Number},
    jac_cache::ForwardColorJacCache,
    jac_prototype = nothing,
...
function forwarddiff_color_jacobian!(
    ...
    forwarddiff_color_jacobian!(
        ...
        f,
        x,
        ForwardColorJacCache(f, x, dx = dx, colorvec = colorvec, sparsity = sparsity),
    )
end
    ...
    for i in eachindex(p)
        ...
        if !(sparsity isa Nothing)
                    ...
                    if J isa AbstractSparseMatrix
                        if J isa SparseMatrixCSC
                            @. void_setindex!(
                                ...
                                (getindex(Ref(adaptedcolorvec), cols_index) == color_i) *
                                getindex(Ref(vecdx), rows_index),
                                rows_index,
                            )
                        else
            ...
            for j = 1:chunksize
                ...
                (col_index > ncols) && return J
                if J isa Array
                    @inbounds @simd for k = 1:size(J, 1)
