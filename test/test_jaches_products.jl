...
cache1 =
    ForwardDiff.Dual{
        typeof(ForwardDiff.Tag(SparseDiffTools.DeivVecTag(), eltype(x))),
        eltype(x),
        1,
...
@test num_jacvec!(dy, f, x, v, similar(v), similar(v)) ≈
...
@test num_hesvec!(dy, g, x, v, similar(v), similar(v), similar(v)) ≈
      ForwardDiff.hessian(g, x) * v rtol = 1e-2
...
@test numauto_hesvec!(
    ...
    x,
    v,
    ForwardDiff.GradientConfig(g, x),
    similar(v),
    similar(v),
) ≈ ForwardDiff.hessian(g, x) * v rtol = 1e-8
...
@test autonum_hesvec!(dy, g, x, v, cache1, cache2) ≈ ForwardDiff.hessian(g, x) * v rtol =
    1e-2
...
@test numback_hesvec!(dy, g, x, v, similar(v), similar(v)) ≈ ForwardDiff.hessian(g, x) * v rtol =
    1e-8
...
@test autoback_hesvec!(dy, g, x, v, cache3, cache4) ≈ ForwardDiff.hessian(g, x) * v rtol =
    1e-8
...
@test num_hesvecgrad!(dy, h, x, v, similar(v), similar(v)) ≈ ForwardDiff.hessian(g, x) * v rtol =
    1e-2
...
@test auto_hesvecgrad!(dy, h, x, v, cache1, cache2) ≈ ForwardDiff.hessian(g, x) * v rtol =
    1e-2
